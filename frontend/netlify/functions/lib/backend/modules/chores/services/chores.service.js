...
export class ChoresRepository {
    ...
    findAudienceUserIds(audienceType, floorId, requesterId, targetCompanyName, targetUserId, requesterCompanyName) {
        switch (audienceType) {
            case 'OPEN':
...
export class ChoresService {
    ...
    choreContext(chore) {
        return {
            ...
            audienceType: chore.audienceType,
    ...
    async validateCreate(requester, data) {
        ...
        switch (data.audienceType) {
            ...
            case 'OTHER_COMPANY': {
                ...
                if (!targetCompany)
                    throw new Error('Odaberite tvrtku.');
            ...
            case 'SPECIFIC_USER': {
                if (!data.targetUserId)
                    throw new Error('Odaberite korisnika.');
                if (data.targetUserId === requester.id)
                    throw new Error('Ne možete odabrati sebe.');
    ...
    async create(requesterId, data, userFloorId) {
        const requester = await this.repo.findUserById(requesterId);
        if (!requester)
            throw new Error('Korisnik nije pronađen.');
        await this.validateCreate(requester, data);
        const requesterCompany = normalizeCompany(requester.profile?.companyName);
        const targetCompanyName = data.audienceType === 'OTHER_COMPANY' ? normalizeCompany(data.targetCompanyName) : null;
        ...
        const audienceUsers = await this.repo.findAudienceUserIds(data.audienceType, dto.floorId, requesterId, targetCompanyName, targetUserId, requesterCompany);
    ...
    async list(viewerId, floorId, status) {
    ...
    listCompanies() {
        return this.repo.listCompanyNames().then((rows) => rows
            .map((row) => normalizeCompany(row.companyName))
            .filter((name) => Boolean(name)));
    }
    async volunteer(id, volunteerId) {
        const [chore, volunteer] = await Promise.all([
            this.repo.findById(id),
            this.repo.findUserById(volunteerId),
        ]);
        if (!chore)
            throw new Error('CO-OP zahtjev nije pronađen.');
        if (!volunteer)
            throw new Error('Korisnik nije pronađen.');
        if (chore.status !== 'OPEN')
            throw new Error('CO-OP više nije otvoren.');
