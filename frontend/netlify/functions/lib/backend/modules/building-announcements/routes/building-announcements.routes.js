...
export function createBuildingAnnouncementsRoutes() {
    const router = Router();
    router.use(authMiddleware);
    router.get('/', buildingAnnouncementsController.list);
    router.post('/', requireRole('BUILDING_ADMIN'), validateBody(createBuildingAnnouncementSchema), buildingAnnouncementsController.create);
    router.delete('/:id', requireRole('BUILDING_ADMIN'), buildingAnnouncementsController.remove);
    return router;
}
