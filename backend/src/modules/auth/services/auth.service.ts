...
import type { AuthUser, TokenPair } from '@building-app/shared';
...
export class AuthService {
  ...
  async register(data: {
    ...
    if (existing) throw new Error('E-pošta je već registrirana');
  ...
  async login(email: string, password: string) {
    ...
    if (!valid) throw new Error('Neispravni podaci za prijavu');
  ...
  private async generateTokens(user: AuthUser): Promise<TokenPair> {
    const accessToken = jwt.sign(
      { id: user.id, email: user.email, role: user.role, floorId: user.floorId },
